...
app.get("/financial-summary/:ticker", async (req, res) => {
  ...
  try {
    ...
    if (!data) {
      return res
        .status(404)
        .json({ error: `No financial data found for ticker: ${ticker}` });
    }
    ...
    res
      .status(500)
      .json({
        error: "Failed to fetch financial summary. Please try again later.",
      });
  }
});
...
app.get("/news/:symbols", async (req, res) => {
  ...
  try {
    ...
    if (!response.data || Object.keys(response.data).length === 0) {
      return res
        .status(404)
        .json({ error: "No news available for the given symbols." });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch stock news. Please try again later." });
  }
});
app.post("/chat", async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Error communicating with Cohere API:",
      error.message,
      error.response?.data
    );
    res
      ...
      .json({
        error:
          "Failed to fetch response from Cohere API. Please try again later.",
      });
  }
});
